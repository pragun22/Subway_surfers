function Dog(gl,x,y,z) {
  ...
  var primitives = [];
  primitives.push(Basic2(gl, 0, 0, 0,0.7/12.0 ,0.7/12.0 ,0.7/12.0))
  primitives.push(Basic2(gl, 0, 0+1.25/12.0, 0,0.5/12.0 ,0.45/12.0 ,0.6/12.0))
  primitives.push(Basic2(gl, 1/12.0, -1/12.0, 0,  0.2/12.0,0.6/12.0,0.6/12.0))
  primitives.push(Basic2(gl, 1/12.0, 1/12.0, 0, 0.2/12.0,0.6/12.0,0.6/12.0))
  primitives.push(Basic2(gl, -1/12.0, -1/12.0 , 0, 0.2/12.0,0.6/12.0,0.6/12.0))
  primitives.push(Basic2(gl, -1/12.0, 1/12.0 , 0, 0.2/12.0,0.6/12.0,0.6/12.0))
  let init = () =>{
  ...
  let tick = () => {
    if( rotation_ob > 0*Math.PI/180.0) t = -0.07;
    if( rotation_ob < -50*Math.PI/180.0) t = 0.07;
    rotation_ob += t;
    if( rot_leg > 0*Math.PI/180.0) t2 = -0.04;
    if( rot_leg < -50*Math.PI/180.0) t2 = 0.04;
    rot_leg += t2;
